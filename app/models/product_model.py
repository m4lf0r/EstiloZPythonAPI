...
class ProdutoDAO:
    ...
    def get(self, id):
        ...
        if produto_data:
            # Garante que o ID está no dicionário retornado
    ...
    def create(self, produto_data):
        global contador_id
        contador_id += 1
        # Não precisa mais adicionar 'id' aqui, pois a resposta do controller
        # usará o get() que já faz isso.
        PRODUTOS_DB[contador_id] = produto_data
        return self.get(contador_id) # Retorna o produto completo com ID

    def update(self, id, produto_data):
        if id in PRODUTOS_DB:
            PRODUTOS_DB[id].update(produto_data)
            return self.get(id) # Retorna o produto completo com ID
        return None
